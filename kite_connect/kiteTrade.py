...
class KiteApp:
    ...
    def holdings(self):
        margins = self.session.get(f"{self.root_url}/portfolio/holdings", headers=self.headers).json()["data"]
        return margins
    ...
    # payload = 
        ...
        # watch_id: number
        # weight: number
    def instrument_info(self, market_watch_id, payload):
        instrument = self.session.post(f"{self.root_url}/marketwatch/{market_watch_id}/items",
                                data=payload, headers=self.headers).json()["data"]
        return instrument